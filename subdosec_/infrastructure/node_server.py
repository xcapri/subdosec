...
from ..core.ports import ServerManager, LocalStorage
from ..shared import colors
from ..shared.utils import is_port_in_use, get_random_unused_port


class NodeServerManager(ServerManager):
    def __init__(self, storage: LocalStorage):
        self.storage = storage

    def get_node_env_dir(self) -> str:
        return os.path.join(self.storage.get_user_dir(), "node_vm")

    def install_node_runtime(self) -> None:
        env_dir = self.get_node_env_dir()
    ...
    def run_node_server(self) -> subprocess.Popen:
        """Start the Node.js server with auto-healing capabilities."""
        # Find path to 'node' folder relative to this file's directory (infrastructure/)
        infra_dir = os.path.dirname(os.path.abspath(__file__))
        package_dir = os.path.dirname(infra_dir)
        
        # Paths
        node_dir = os.path.join(package_dir, 'node')
        ...
        _, _, _, host_scan_prod, node_port = self.storage.load_env_vars('public')

        # 2. Ensure Node.js Environment
        node_exe, npm_exe = self.get_node_paths()
        
        if not node_exe:
             print(colors.warning("Node.js executable not found. Attempting auto-install..."))
             try:
                 self.install_node_runtime()
                 node_exe, npm_exe = self.get_node_paths()
        ...
        try:
            if not os.path.exists(node_modules_dir):
                ...
                try:
                    # Add node executable path to PATH environment variable for npm
                    env = os.environ.copy()
